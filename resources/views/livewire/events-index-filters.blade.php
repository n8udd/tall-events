<div id="main-filters" class="bg-white shadow-md p-6 mb-4">
    ...
    <div class="mb-2">
        ...
        <div class="flex space-x-2">
            ...
            <button wire:click.prevent="setLevel('{{$levelName}}')" class="flex pointer p-2 text-sm text-center justify-center bg-{{$colours[$levelName]}}-600 hover:bg-{{$colours[$levelName]}}-900 @if($level === $levelName) bg-{{$colours[$levelName]}}-900 font-bold @endif">{{ $levelName }} <br> ({{ $levelCount }})</button>
            @endforeach
        </div>
    </div>
    {{-- end difficulty --}}
