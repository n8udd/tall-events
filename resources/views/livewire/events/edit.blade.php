<div class="w-full">
    ...
    <form action="{{ route('events.update',$event) }}" method="post" class="w-full pt-8">
        @csrf
        @method('PUT')
                    ...
                    <div class="flex flex-col mt-6">
                        ...
                        <div x-data="{ count: 0 }" x-init="count = $refs.count_intro.value.length">
                            <textarea id="intro" name="intro" rows="2"
                                ...
                                x-on:keyup="count = $refs.count_intro.value.length"
                                required>{{ $event->intro ?: NULL}}</textarea>
                        ...
                        <div x-data="{ count: 0 }" x-init="count = $refs.count_desc.value.length">
                            <div class="mt-1">
                                <textarea id="description" name="description" rows="5"
                                    ...
                                    x-on:keyup="count = $refs.count_desc.value.length"
                                    required>{{ $event->description ?: NULL }}</textarea>
                            </div>
                            <div class="items-end">
                                <div class="mt-2 text-sm text-gray-300">
                ...
                <div class="flex flex-col px-4 py-5 bg-white sm:p-6 w-full">
                    ...
                    <div class="flex flex-row w-full space-x-6">
                        <div class="flex flex-col w-1/2">
                            ...
                            <select name="level_id" id="level_id"
                                ...
                                <option value="{{$level->id}}" class="dark:text-black" @if($level->id ==
                                    $event->level_id) selected="selected" @endif>
                                    {{ucfirst($level->name)}}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex flex-col w-1/2">
                            ...
                            <select name="type_id" id="type_id"
                                ...
                                <option value="{{$type->id}}" @if($type->id ===
                                    $event->type_id) selected="selected" @endif>{{ucfirst($type->name)}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="flex flex-row">
                            <div class="flex">
                                <label class="inline-flex items-center my-2 mr-16">
                                    <input type="radio" class="form-radio h-6 w-6 ml-6 md:ml-0 text-blue-400"
                                        name="leader_led" value="1" required @if($event->leader_led) checked @endif>
                                    <span class="ml-2 text-gray-500">Yes</span>
                                </label>
                            </div>
                            <div class="flex">
                                <label class="inline-flex items-center my-2">
                                    <input type="radio" class="form-radio h-6 w-6 ml-6 text-red-400" name="leader_led"
                                        value="0" required @if(!$event->leader_led) checked @endif>
                    ...
                    <div class="flex flex-col mt-6 ml-2">
                        ...
                        <div class="flex flex-col md:flex-row">
                            <div class="flex">
                                <label class="inline-flex items-center my-2 mr-16">
                                    <input type="radio" class="form-radio h-6 w-6 ml-6 md:ml-0 text-green-400"
                                        name="gender" value="mixed" required="required" @if($event->gender == 'mixed')
                                    checked @endif>
                                    <span class="ml-2 text-gray-500">Mixed</span>
                                </label>
                            </div>
                            <div class="flex">
                                <label class="inline-flex items-center my-2 mr-16">
                                    <input type="radio" class="form-radio h-6 w-6 ml-6 text-pink-400" name="gender"
                                        value="ladies" @if($event->gender == 'ladies')
                                    checked
                                    @endif>
                                    <span class="ml-2 text-gray-500">Ladies</span>
                                </label>
                            </div>
                            <div class="flex">
                                <label class="inline-flex items-center my-2 mr-16">
                                    ...
                                    checked
                                    @endif>
                                    <span class="ml-2 text-gray-500">Gents</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-row w-full space-x-6">
                        <div class="flex flex-col w-1/2">
                            ...
                            <input type="date" name="start_date" id="start_date"
                                ...
                                placeholder="dd-mm-yyyy" value="{{ $event->start_date->format('Y-m-d') ?: NULL }}"
                                required>
                        </div>
                    ...
                    <div class="flex flex-row w-full space-x-6 mt-6">
                        <div class="flex flex-col w-1/2">
                            <label for="min_attendees" class="block text-sm font-medium text-gray-500">Min
                                Attendees
                                <input type="number" name="min_attendees" id="min_attendees"
                                    value="{{ $event->min_attendees ?: NULL }}"
                                    class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:text-gray-900">
                            </label>
                        </div>
                        ...
                        <div class="flex flex-col w-1/2">
                            <label for="max_attendees" class="block text-sm font-medium text-gray-500">Max
                                ...
                                <input type="number" name="max_attendees" id="max_attendees"
                                    value="{{ $event->max_attendees ?: NULL }}"
                                    class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:text-gray-900">
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col w-full mt-6">
                        ...
                        <div class="flex flex-row">
                            <div
                                ...
                                https://</div>
                            <input type="url" name="url" id="url" maxlength="50" placeholder="url..."
        ...
        <div class="w-full flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
            <div class="w-full">
                <button wire:click.prevent="cancelEdit()"
                    class="flex justify-center p-2 border border-transparent shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 w-full">
                    cancel edit
                </button>
            </div>
            <div class="w-full">
